...
async function autoPurchase(orderRef) {
  // AliExpress cannot deliver to China (export-only) nor to Mauritania, so
  // automatic purchasing is disabled by default. The Guangzhou team sources
  // confirmed orders domestically (1688/Taobao) using the AE links shown in
  // the admin order cards. Set AE_AUTO_PURCHASE=1 to re-enable the automation.
  if (process.env.AE_AUTO_PURCHASE !== "1") return;
  let order;
  try {

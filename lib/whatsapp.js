...
async function sendWhatsAppOtp(phone, code) {
  ...
  if (!token || !phoneId) {
    // Test mode is only allowed while OTP_TEST_MODE isn't explicitly disabled.
    // Set OTP_TEST_MODE=0 in Vercel to hard-disable on-screen codes (e.g. at
    // launch time if WhatsApp still isn't configured).
    if (process.env.OTP_TEST_MODE === "0") {
      console.error("OTP requested but WhatsApp is not configured and test mode is disabled.");

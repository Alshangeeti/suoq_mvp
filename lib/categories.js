...
const MAIN_SLUGS = CATEGORY_TREE.map((c) => c.slug);

function findCategory(slug) {
  return CATEGORY_TREE.find((c) => c.slug === slug) || null;
}

function isValidSub(catSlug, subSlug) {
  const cat = findCategory(catSlug);
  return !!(cat && cat.subs.some((s) => s.slug === subSlug));
}

module.exports = { CATEGORY_TREE, MAIN_SLUGS, findCategory, isValidSub };
